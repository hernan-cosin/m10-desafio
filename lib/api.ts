...
export async function searchProducts(
  ...
  const resProducts = await fetchAPI(
    "/search?q=" + query + pageParam + hitsPerPageParam,
    {
      method: "GET",
    }
  );
...
export async function generateOrder(info: orderInfo) {
  const aditionalData = JSON.stringify({
    amount: info.amount,
    flavor: info.flavor,
  });
...
export async function updateUserInfo(userData: any) {
  const token = getSavedToken();
